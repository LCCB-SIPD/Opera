...
async function POST(req) {
    ...
    try {
        ...
        const [rows] = await connection.execute("SELECT * FROM user_tbl WHERE username = ?", [
            username
        ]);
        if (rows.length === 0) {
        ...
        const isValid = await __TURBOPACK__imported__module__$5b$externals$5d2f$bcrypt__$5b$external$5d$__$28$bcrypt$2c$__cjs$29$__["default"].compare(password, rows[0].password);
        if (isValid) {
        ...
        console.error(error);
        return new Response("Error with database connection", {
            status: 500
        });
    }
}
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__045c482f._.js.map